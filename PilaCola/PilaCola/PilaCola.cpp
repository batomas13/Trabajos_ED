...
{
	Nuevo->Siguiente = Lista; // Crea un nuevo nodo en la lista a la que se le asigna un nuevo articulo
	Lista = Nuevo;
}
void AgregarFinalInventario(PtrTArticulo& Lista, PtrTArticulo& Nuevo)
{
	...
	if (Aux != NULL)
	{
		...
		Aux->Siguiente = Nuevo;  // Cuando llega la ultimo nodo le asigna "nuevo"
	}
	else
	{
		Lista = Nuevo;
...
void Push(PtrTArticulo& Lista, PtrTArticulo Nuevo) {
	AgregarInicioInventario(Lista, Nuevo);  // Usa la funcion para agregar al inicio
}
